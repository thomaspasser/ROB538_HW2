...
int main(){
  ...
  std::cout << "Running Q-learning for problem 2" << std::endl;
  ...
  domain.run(totalIterations, totalIterations/100, &stepsArray, a, true, false);
  ...
  domain.saveData("problem2_distdata");

  //domain.run(10, 10, &stepsArray2, a, false, false, true);
  ...
  */
  ...
  // save Q to json
  a1.print("agent2_1.json");
  a2.print("agent2_2.json");

  // save training stats to csv
  std::ofstream csvfile;
  csvfile.open("training_stats_2.csv");
